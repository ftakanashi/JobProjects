...
### 解法 模拟
思路没啥可说的，很明显的模拟题。
不过还是得注意一些细节，比如皇后之间是会互相遮挡的，这也就意味着当同一条直线上有多个皇后的时候，只有距离国王最近的那个皇后才可以被收割进答案。

一开始我想直接遍历所有皇后然后判断和国王在不在同一行同一列或者同一个对角线上，因为这个限制，显然就不能这么做了。

换个视角，我们可以从国王出发，分别向同一行同一列同一对角线的8个方向遍历探索。直到越界或者碰到皇后将其收割进答案即可。