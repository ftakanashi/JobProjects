...
后根序是中根序的扩展。可以记住"开栈空，左到底，右先讨论收割"，并且要额外设置prev指针。
前两个操作和中根序是一样的。
但是在右子树的操作时，存在两种情况。
...
>！！！注意！！！
>以上讨论过程很容易先讨论了"否则"的情况，即代码写成了`if prev is node.left and node.right`。但实际上
>prev这个指针只保证prev是node.right的时候，node可以被收割。（比如prev是左子树靠右下的节点时，按照我们的想法应该是上述prev is node.left，但是并不是。
>所以分类讨论的if条件必须写`if node.right is None or prev is node.right`。
>
>即 分类讨论必须先讨论收割node的情况。
