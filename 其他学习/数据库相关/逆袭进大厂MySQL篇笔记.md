...
==一个表，有且只有一个聚簇索引，并且索引结构在建表时就直接建立==。当建表语句中指定了主键，自然是以上面的主键索引的形式建立聚簇索引。若表没有定义主键，则使用表定义中的第一个非空唯一字段列作为索引列构建聚簇索引。若还是没有这样的列，则存储引擎会创建隐藏row-id作为依据，建立聚簇索引。
...
面试中经常会问到的MySQL中的三种日志，binlog，undo log，redo log。
